...
def step1_unpack(cfg: Config) -> bool:
    ...
    if cfg.unpacked_dir.exists():
        key_files = ['table_weapons', 'table_armor', 'table_items_stats', 'table_attributes', 'table_mobs', 'table_mobs_stats']
        ...
        if v:
            ...
            if v_id:
                ...
                if src_png_v.exists():
                    ...
                    try:
                        Image.open(src_png_v).save(str(dest_webp_v), 'WEBP', quality=90, lossless=False)
...
# Step 6: 转换为怪物 JSON
# ============================================================================
def parse_mobs_translations(cfg: Config) -> Tuple[Dict[str, Dict[str, str]], Dict[str, Dict[str, str]]]:
...
def categorize_mob_stats(row: Dict[str, str]) -> Dict[str, Any]:
    ...
    for key, value in row.items():
        ...
        elif key not in ['name', 'Tier', 'ID', 'type', 'faction', 'pattern',
                        'category1', 'category2', 'size', 'matter', 'Checksum', '',
                        'achievement', 'trophy', 'Threat_Time']:
            other[key] = num_val
    ...
    if offense:
        ...
        for k, v in offense.items():
            if k in ['slashing', 'piercing', 'blunt', 'rending', 'fire', 'shock',
                    'poison', 'caustic', 'frost', 'arcane', 'unholy', 'sacred', 'psionic']:
                damage_types[k] = v
            else:
                regular_fields[k] = v
...
def generate_search_keywords(name_en: str, name_zh: str, faction: str, 
                            mob_type: str, pattern: str, category: str) -> List[str]:
    """生成搜索关键词"""
    keywords = []
...
def step6_convert_mobs_to_json(cfg: Config) -> bool:
    ...
    with open(mobs_stats_file, 'r', encoding='utf-8-sig') as f:
        ...
        for row in reader:
            ...
            keywords = generate_search_keywords(name_en, name_zh, faction, 
                                               mob_type, pattern, category1)
